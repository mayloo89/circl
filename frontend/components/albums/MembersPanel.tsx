...
import { albumsApi, type Grant } from "@/lib/albums"
...
export default function MembersPanel({ albumID, token }: Props) {
  ...
  const openGranteeIDs = new Set(
    grants.filter((g) => g.status === "pending" || g.status === "active").map((g) => g.grantee_id),
  )
  ...
  async function invite() {
    ...
    setError("")
    try {
      const grant = await albumsApi.invite(token, albumID, selected)
  ...
  const open = grants.filter((g) => g.status === "pending" || g.status === "active")
      ...
      {open.length === 0 ? (
        ...
        <ul className="space-y-2">
          {open.map((g) => {
            ...
            const isInvitePending = g.status === "pending" && g.source === "invite"
            ...
            return (
              ...
              >
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  {(g.status === "active" || isInvitePending) && (
