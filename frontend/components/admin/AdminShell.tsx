...
const NAV_HREFS = [
  ...
  { href: "/admin/appeals", key: "appeals" },
  { href: "/admin/moderation", key: "moderation" },
  { href: "/admin/channels", key: "channels" },
  { href: "/admin/public-rooms", key: "publicRoomsNav" },
] as const
    ...
    <div className="flex min-h-screen flex-col bg-gray-950">
      ...
      >
        <div className="flex items-start justify-between border-b border-gray-800 px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs font-medium uppercase tracking-wider text-brand-muted">{t("adminBadge")}</p>
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
      ...
      <header className="flex items-stretch border-b border-gray-800 bg-gray-900 px-4">
        ...
        <div className="flex shrink-0 items-center gap-3 py-3 pr-6">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-brand-muted">{t("adminBadge")}</span>
        </div>

        {/* Desktop horizontal tab nav */}
        <nav className="hidden items-stretch gap-1 md:flex" aria-label={t("adminBadge")}>
          {NAV_HREFS.map(({ href, key }) => {
            const active = href === "/admin"
              ? pathname === "/admin"
              : pathname === href || pathname.startsWith(href + "/")
