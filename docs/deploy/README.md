...
| `moderation`  | `../ops/moderation`      | 8000          | no      |
| `backend`     | `../backend`             | 8080          | 127.0.0.1:8080 |
| `frontend`    | `../frontend`            | 3000          | 127.0.0.1:3000 |
...
- **`docker-compose.prod.yml`** — production compose. Build contexts point
  at `../backend`, `../frontend`, `../ops/moderation`, so the file expects
  to be one level deep relative to the repo (e.g. `deploy/`).
...
The first build downloads PyTorch + the NudeNet ONNX model for the
moderation sidecar — expect 15–25 min on a modest single-board host, faster
on a beefy VPS.
...
```bash
cd /opt/circl
git pull origin main                                    # or develop, whatever you track

# IMPORTANT: --build is required when there are code changes. Without it,
# the containers reuse the previously-built image and the new code never
# ships even though git is updated. `restart` alone is NOT enough.
docker compose --env-file deploy/.env.prod -f deploy/docker-compose.prod.yml up -d --build
```
