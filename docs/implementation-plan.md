...
- [ ] **Expanded report categories** — `csam`, `non_consensual_intimate_images` (Ley Olimpia), `digital_gender_violence` (Ley Olimpia), `threat_violence`, `impersonation`, `underage`. CSAM and NCII auto-page on-call and bypass the normal queue.
- [ ] **Age-verification audit log + policy doc** — log "claims 18+" with timestamp + IP at registration; document the policy under `/safety`. Document-based verification (Stripe Identity / Veriff) deferred — add when global regulatory pressure (UK OSA, US state laws) or report rate justifies the per-user cost.
...
- [ ] **Image moderation in the upload worker** — three layers, all applied to every image upload regardless of surface (avatar, gallery, channels, DMs, private albums):
  - **CSAM** — Cloudflare CSAM Scanning Tool (free, easy onboarding) or PhotoDNA (Microsoft, free but requires NCMEC application). Match → upload rejected, account suspended, NCMEC report filed.
  - **NCII** — apply to **StopNCII.org** as a participating platform; perceptual-hash match against victim-submitted hashes. Match → upload rejected.
  - **NSFW classifier** — NudeNet (open source, runs inside the asynq worker) or AWS Rekognition (paid). Used to **reject** explicit nudity from avatar / gallery / channels; used to **tag-not-block** in DMs / private albums so the moderation queue can prioritise.
  - Rejected uploads return `code=upload_rejected_moderation`; admin notified.
- [ ] **Habeas Data / GDPR Art. 20 data export** — `POST /account/export` enqueues an asynq job → generates a JSON + media zip in S3 → emails a one-time signed URL with 14-day retention. Export must include the private-album grants log so users can produce evidence of consent.
- [ ] **Private albums with consented sharing** — owner uploads explicit content to a named album (passes through the full CSAM/NCII pipeline; NSFW tag-not-block); explicit per-user grants in `private_album_grants` (`granter_id`, `grantee_id`, `granted_at`, `revoked_at`) audit every consent event; backend issues short-TTL signed URLs only when an active grant exists; revoke is forward-only (UI must say so — viewer's browser cache cannot be invalidated retroactively); server-side watermark composites viewer's username + timestamp into the rendered image; mutual-unlock pattern (Feeld-style) optional; rate-limit grants to ~20/day to block bulk-grant abuse; access log (`private_album_views`) for legal defense and user accountability. Depends on the image-moderation pipeline being live.
