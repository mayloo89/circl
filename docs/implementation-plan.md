...
- [x] **Private albums — hardening**: server-side deterrence watermark (`compositeWatermark` in `internal/albums/watermark.go`) composites viewer display name + UTC timestamp at the bottom-right of every photo served to a non-owner caller using `golang.org/x/image/font/gofont/gomono` (no cgo); JPEG and PNG handled natively, WebP decoded and re-encoded as JPEG; owners receive raw bytes. `ProfileNameFunc` option on `albums.NewHandler` resolves display names without coupling to the profiles package. 20-invitations/day rate limit on `POST /albums/{id}/grants/invite` via `ratelimit.RedisLimiter`; errors fail open. Admin access log at `GET /admin/albums/views` (paginated, filterable by album_id) backed by new `Store.ListViews` (joins `private_albums` + `profiles`); mounted via `admin.WithAlbumsHandler`; frontend page at `/admin/albums` with event-type chips and pagination. 5 new backend tests. EN/ES/PT i18n.
