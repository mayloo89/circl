...
- [ ] **Private albums — hardening (follow-up PR)**: server-side watermark compositing viewer's username + timestamp into each render; mutual-unlock pattern (Feeld-style — both parties opt-in independently); 20-grants/day rate limit per granter; admin-side review surface for the access log.
