...
- [ ] **Vendor integrations to plug into the moderation pipeline** (no code change required to the orchestration layer once these land):
  ...
  - **NSFW classifier** — NudeNet (open source, runs inside the asynq worker via ONNX runtime or a Python sidecar) or AWS Rekognition (paid). Implement `NSFWClassifier`; `NSFW` moderator already wires it. Used to **reject** explicit nudity from avatar / gallery / channels; used to **tag-not-block** in DMs / private albums (the moderator can return `tag` decisions in a future extension).
