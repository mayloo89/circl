...
- [x] **"Don't show my distance to non-contacts" toggle** in `/settings` — `Service.Browse` now batch-loads each browsed profile's `hide_distance_from_non_contacts` flag plus the caller's accepted contacts and nulls `distance_km` for non-contact viewers when the flag is set. Migration `000027` introduced the column. Public profile (`GET /profiles/{ref}`) already strips coordinates so no extra gate was needed.
- [x] **Hide presence / last-seen toggle** in `/settings` — symmetric. `presence.NewHandler` takes a `PrivacyLookup`; `getPresenceHandler` forces `online=false` + drops `last_seen_at` when the viewed user hides, and for every entry when the caller hides. `heartbeatHandler` / `offlineHandler` fan out SSE via `fanoutPresence` which drops the event entirely if the transitioning user hides and skips per-recipient when a contact hides.
- [x] **Read-receipts and typing-indicator opt-out** — symmetric. `chat.HandlerConfig` gained a `PrivacyResolver`; `hub.deliver` parses each frame's `event` and skips `typing` / `read_receipt` for clients with the matching flag. `readPump` gates the `typing` emit on `c.hideTyping`; `NotifyRoomRead` gates the `read_receipt` publish on the marking user's `HideReadReceipts`. Mid-session toggles take full effect on the next WS reconnect (Client struct holds the snapshot taken at connect time).
...
- [ ] **Latent merge bug — `PUT /profiles/me/preferences`** is REPLACE, not MERGE. The locale-only PUT in `LanguageSection` and `Sidebar` clobbers `min_age` / `max_age` / `max_distance_km` / `gender_preference` to NULL. The new `PrivacySection` works around it by fetching the full object first, but the underlying `UpsertPreferences` should switch to COALESCE-on-NULL semantics (or the locale-only callers should fetch-then-PUT) in a follow-up.
