...
- [x] **"Looking for" public-profile fields** — migration `000030` adds the four columns to `profiles`. Backend exposes a constrained tag set (`chatting`, `dating`, `friendship`, `language_exchange`) and the same gender set the browse filter uses (`Man`, `Woman`, `Non-binary`, `Other`). Validation rejects unknown tags / genders, age values outside 18–120, and `min > max`. Both `toResponse` (owner) and `toPublicResponse` (non-owner) include the fields since they're explicitly public. Profile edit gains a "Looking for" card with three sub-rows (tag chips, gender chips, min/max age inputs); public profile renders a chip block when at least one field has a value, formatting the age range as `min–max` / `min+` / `up to N`. EN/ES/PT i18n. The "Copy from search filters" convenience button is intentionally deferred to a follow-up.
