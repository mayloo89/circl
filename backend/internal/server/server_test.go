...
func TestHealthHandler_DBOk(t *testing.T) {
	h := server.New(&mockPinger{}, "test", []string{"http://localhost:3000"}, http.NotFoundHandler(), http.NotFoundHandler(), http.NotFoundHandler(), http.NotFoundHandler(), func(h http.Handler) http.Handler { return h })
...
func TestHealthHandler_DBError(t *testing.T) {
	h := server.New(&mockPinger{err: errors.New("connection refused")}, "test", []string{"*"}, http.NotFoundHandler(), http.NotFoundHandler(), http.NotFoundHandler(), http.NotFoundHandler(), func(h http.Handler) http.Handler { return h })
...
func TestHealthHandler_ContentType(t *testing.T) {
	h := server.New(&mockPinger{}, "production", []string{"*"}, http.NotFoundHandler(), http.NotFoundHandler(), http.NotFoundHandler(), http.NotFoundHandler(), func(h http.Handler) http.Handler { return h })
