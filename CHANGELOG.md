...
- **"Looking for" public-profile fields** ([PR #100](https://github.com/mayloo89/circl/pull/100)): new section on the profile edit page lets users state who and what they're looking for, distinct from the private `profile_preferences` discovery filters that filter their browse view. Backed by migration `000030` which adds four columns to `profiles` (`looking_for_tags TEXT[]`, `looking_for_gender TEXT[]`, `looking_for_age_min INT`, `looking_for_age_max INT`, all nullable / empty by default). Tag set is constrained to `chatting`, `dating`, `friendship`, `language_exchange`; gender uses the same canonical set as the browse filter (`Man`, `Woman`, `Non-binary`, `Other`) so the surfaces stay aligned. Backend validates unknown tags / genders, age values outside 18–120, and `min > max`. Both the owner-view and non-owner-view profile responses include the fields (they're explicitly public — the whole point is for others to see them). Profile edit UI: chip toggles for tags + genders, two number inputs for the age range. Public profile rendering shows a "Looking for" chip block only when at least one field has a value, formatting the age range as `min–max` / `min+` / `up to N`. Five service-layer tests cover the validation paths (unknown tag, too many tags, unknown gender, out-of-range / inverted age, all-canonical-values happy path). Full EN/ES/PT i18n. The "Copy from search filters" button is deferred to a follow-up if usage telemetry shows it'd be useful.
