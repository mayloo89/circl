...
- **Settings toggles no longer scroll the page on click** ([PR #94](https://github.com/mayloo89/circl/pull/94)): the `Toggle` primitive was rendering its native `<input>` as `sr-only`, which positions a 1×1 box at the top-left of the surrounding `<label>`. AppShell wraps content in `<main className="flex-1 overflow-auto">` (the actual scroll container), so when the input received focus on toggle, the browser scroll-into-view behavior jumped `<main>` to bring the off-viewport input box into view — exposing a visible blank gap below the content. The input is now positioned absolutely (transparent, `inset-0`) over the visible track, so its bounding box matches the track and is already in view; focus no longer triggers a scroll. The settings page wrapper switched from `min-h-screen` (= 100vh, ignores the `<main>` scroll container) to `min-h-full` (fills `<main>`'s actual height) so any residual scroll-into-view can't expose overscroll room.
