...
- ✅ **Mobile UX** ([PR #85](https://github.com/mayloo89/circl/pull/85)): iOS safe-area insets on `TopBar`/`BottomNav`/`AppShell`/`Toast`/profile sticky bar; `text-base` on inputs prevents iOS auto-zoom; auto-growing `<textarea>` chat composer (Enter to send, Shift+Enter for newline); `capture` attributes on file inputs; push prompt deferred until first conversation; 44 pt touch targets on bell, close, and dismiss buttons
