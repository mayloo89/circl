...
- ✅ **`NEXT_LOCALE` cookie hardening fix** (Wapiti scan finding): the cookie-hardening re-application in `proxy.ts` read the cookie back through `res.cookies.get("NEXT_LOCALE")`, but next-intl sets it via a raw `Set-Cookie` header, so the read never matched and the `httpOnly`/`secure`/`sameSite` flags never actually applied. The locale is now derived from the request pathname directly and the cookie is always (re)written with the hardened flags plus a one-year `maxAge`.
