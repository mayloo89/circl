...
- ✅ **Privacy controls UI — manage blocked users in Settings**: a dedicated "Blocked users" section in `/settings` lists everyone the user has blocked (avatar + display name + Unblock per row), with a localized empty state; the blocked-users list is removed from `/contacts` so Settings is the canonical home for account-level privacy management; the unblock confirm dialog and `DELETE /contacts/{id}/block` flow are unchanged; the public-profile overflow-menu unblock path was state-walked across all peer-profile states and confirmed correct in every state
