...
- ✅ **Desktop UX + accessibility — phase 1** (PR #87): desktop two-column DM layout via reusable `ChatListPane` (variants `page` / `pane`) on `/chat` and `/chat/[roomId]` for DMs/groups only — channels stay single-pane to make leaving a conscious, confirmed action; skip-to-content link in `AppShell`; `<main id="main-content">` landmark; channel leave copy hardened across EN/ES/PT to spell out irreversible message-access loss
